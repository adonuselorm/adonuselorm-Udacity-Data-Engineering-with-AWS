...
StepTrainerLanding_node1 = glueContext.create_dynamic_frame.from_options(
    ...
    format="json",
    connection_options={
        "paths": ["s3://stedi-lake-house-selormadonu/step_trainer/landing/"],
        "recurse": True,
    },
...
CustomerCurated_node1690332858777 = glueContext.create_dynamic_frame.from_options(
    format_options={"multiline": False},
    connection_type="s3",
    format="json",
    connection_options={
        "paths": ["s3://stedi-lake-house-selormadonu/customer/curated/"],
        "recurse": True,
    },
    transformation_ctx="CustomerCurated_node1690332858777",
)
...
RenamedkeysforPrivacyFilter_node1690332994920 = ApplyMapping.apply(
    frame=CustomerCurated_node1690332858777,
    mappings=[
...
StepTrainerTrusted_node1690333127288 = glueContext.write_dynamic_frame.from_options(
    ...
    format="json",
    connection_options={
        "path": "s3://stedi-lake-house-selormadonu/step_trainer/trusted/",
        "partitionKeys": [],
    },
